...
const App = () => {
  ...
  const removeFromCart = (item) => {
    ...
    const totals = newCartItems.reduce(
      (obj, item) => {
        obj.totalQuantity += item.cartQuantity;
        obj.subTotal += item.price * item.cartQuantity;
        return obj;
  ...
  const handleNewFish = async (createdFish) => {
    console.log("createfish", createdFish);
  ...
  const handleEdit = async (editedFish) => {
    ...
    if (res.ok) {
      ...
      const fishIndex = fishList
        .map((fish) => fish._id === updatedFish._id)
        .indexOf(true);
    ...
    <ThemeProvider theme={theme}>
      <div>
        ...
        <Routes>
          ...
          <Route
            path="/checkout"
            element={fishList && <Checkout fishList={fishList} cart={cart} />}
          />
          <Route
            path="/edit/:fishID"
            element={
              <ProtectedRoute authorised={authorised}>
                {fishList && (
                  <Edit fishList={fishList} handleEdit={handleEdit} />
                )}
              </ProtectedRoute>
            }
          />
          <Route
            path="/signin"
            element={
              <SignIn handleLogin={handleAuth} authorised={authorised} />
            }
          />
          <Route
            path="/signup"
            element={
              <SignUp handleRegister={handleAuth} authorised={authorised} />
            }
          />
        </Routes>
        <Footer />
